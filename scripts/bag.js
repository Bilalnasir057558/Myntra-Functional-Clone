...
function displayBagItems() {
  ...
  bagItemObjects.forEach((item) => {
    innerHtml += generateBagItem(item);
  });
...
function displayBagSummary() {
  ...
  let noOfItems = bagItemObjects.length;
  let totalMRP = 0;
  let discount = 0;
  ...
  let totalPayment = totalMRP - discount + CONVENIENCE_FEE;
...
function removeFromBag(itemId) {
  //removing id from bag items
  bagItems = bagItems.filter((productId) => productId != itemId);

  //again loading bagObjects array
  loadBagItemObjects();

  //again displaying cart items again removing an object
  displayBagItems();

  // changing bag summary section
  displayBagSummary();

  //changing local storage after removing
  localStorage.setItem("bagItems", JSON.stringify(bagItems));

  //changing the bag items count on cart icon
